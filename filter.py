...
def get_processed_frames(rgb_file):
    global total_number_of_frames
    with open(rgb_file, "rb") as file:
...
class CorrelationMapDisplayer:
    def __init__(self, frames, dir, mouse):
        self.fig = plt.figure()
    ...
    def onclick(self, event):
        ...
        nan_loc = np.isnan(self.image)
        print str(nan_loc)
        self.image[nan_loc] = 0
        self.imgplot = self.ax.imshow(self.image)
        ...
        self.canvas.draw()
        save_to_file(self.dir, "Maps/"+self.mouse+"_map_" + str(self.count) + ".raw", self.image, np.float32)
        self.count += 1
